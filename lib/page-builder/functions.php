...
// check if value exists, if not return false
function _check_field_value() {
...
function _s_get_textarea( $text ) {
	...
	return apply_filters( 'pb_the_content', $text );
}


function pb_get_cta_button( $button = array(), $prefix = '' ) {
	
	$button_text =  $button[$prefix.'text'];
	$button_link =  $button[$prefix.'link'];
	$choose_page =  $button[$prefix.'page'];
...
function grid_columns( $columns = array() ) {
